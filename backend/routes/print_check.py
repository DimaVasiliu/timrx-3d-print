    ...
    try:
        ...
        try:
            if USE_DB:
                with get_conn() as conn:
                    with conn.cursor() as cur:
                        # Check job metadata for stage info
                        cur.execute("""
                        ...
                        if row:
                            stage_val = row[0] or ""
                            action_val = row[1] or ""
                            source_action = action_val
                            if "remesh" in action_val:
                                model_stage = "remeshed"
                                is_remeshed = True
                            elif "refine" in action_val or stage_val == "refine":
                                model_stage = "refined"
                                is_refined = True
                            elif stage_val == "preview":
                                model_stage = "preview"
                            elif stage_val == "image3d":
                                model_stage = "image3d"
                            elif "retexture" in action_val:
                                model_stage = "retextured"
                            else:
                                model_stage = stage_val or "unknown"
        except Exception:
