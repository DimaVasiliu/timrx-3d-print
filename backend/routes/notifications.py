...
# ─── Campaign notification click tracking ────────────────────────────────────

@bp.route("/notifications/<notification_id>/click", methods=["POST", "OPTIONS"])
def track_notification_click(notification_id):
    @with_session
    def _inner():
        if request.method == "OPTIONS":
            return ("", 204)
        ...
        try:
            ...
            result = NotificationCampaignService.record_click(notification_id, identity_id)
            return jsonify({"ok": True, **result})
        except Exception as e:
            logger.error("[NOTIF_ROUTE] track_click error: %s", e)
            return jsonify({"ok": True, "clicked": False, "credits_granted": 0})
