...
def _ensure_public_image_url(image_data: str) -> str:
    ...
    return ensure_public_image_url(image_data, provider_name="fal_seedance")
...
class FalSeedanceProvider:
    ...
    def check_status(
        ...
        print(
            f"[FAL_SEEDANCE] check_status called upstream={request_id} "
            f"model_id={model_id or 'NONE'} "
            f"status_url={status_url or 'NONE'} "
            f"response_url={response_url or 'NONE'}"
        )
        try:
            result = check_fal_seedance_status(
                request_id,
                model_id=model_id,
            ...
            print(
                f"[FAL_SEEDANCE] check_status result upstream={request_id[:12]}... "
                f"status={result.get('status')} provider_status={result.get('provider_status')}"
            )
            return result
        except Exception as e:
            print(f"[FAL_SEEDANCE] check_status EXCEPTION upstream={request_id}: {e}")
            _tb.print_exc()
            raise
