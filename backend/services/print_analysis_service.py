...
def _env_int(name: str, default: int, minimum: int | None = None) -> int:
    ...
    try:
        ...
        logger.warning("[PRINT_ANALYSIS] Invalid %s=%r; using default %s", name, raw, default)
        return default
    if minimum is not None and value < minimum:
        logger.warning("[PRINT_ANALYSIS] %s=%s is below minimum %s; using %s", name, value, minimum, minimum)
        return minimum
    return value
...
def _set_analysis_child_limits(memory_mb: int) -> None:
    ...
    if memory_mb < 1024:
        logger.warning(
            "[PRINT_ANALYSIS] Skipping RLIMIT_AS for %sMB cap; too low for trimesh/numpy imports",
            memory_mb,
        )
        return
    try:
...
class PrintAnalysisService:
    ...
    MAX_FACE_COUNT = _env_int("PRINT_ANALYSIS_MAX_FACES", 200000, minimum=1000)
    SCENE_CONCAT_FACE_LIMIT = _env_int("PRINT_ANALYSIS_SCENE_CONCAT_FACE_LIMIT", 120000, minimum=1000)
    WALL_THICKNESS_FACE_LIMIT = _env_int("PRINT_ANALYSIS_WALL_FACE_LIMIT", 25000, minimum=1000)
    WALL_THICKNESS_MAX_SAMPLES = _env_int("PRINT_ANALYSIS_WALL_MAX_SAMPLES", 300, minimum=10)
    WALL_THICKNESS_MIN_SAMPLES = _env_int("PRINT_ANALYSIS_WALL_MIN_SAMPLES", 80, minimum=10)
    ...
    MAX_DOWNLOAD_BYTES = _env_int("PRINT_ANALYSIS_MAX_DOWNLOAD_MB", 20, minimum=1) * 1024 * 1024
    ...
    ANALYSIS_TIMEOUT = _env_int("PRINT_ANALYSIS_TIMEOUT_SECONDS", 30, minimum=30)
    ANALYSIS_MEMORY_LIMIT_MB = _env_int("PRINT_ANALYSIS_MEMORY_MB", 1800, minimum=0)
