...
def backfill_models(cur, rows: list[dict], apply: bool) -> None:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        try:
            ...
            glb_s3_url, content_hash, glb_s3_key_from_upload, _reused = _unpack_upload_result(upload_result)
...
def backfill_images(cur, rows: list[dict], apply: bool) -> None:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        try:
            ...
            image_s3_url, content_hash, image_s3_key_from_upload, _reused = _unpack_upload_result(upload_result)
